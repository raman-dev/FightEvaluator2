...
def get_matchup_comparison(request,matchupId):
    ...
    data = {
        ...
        'fighter_a_notes': [ model_to_dict(n) for n in Note.objects.filter(assessment=Assessment.objects.get(fighter=matchup.fighter_a)).order_by('-createdAt')],
        'fighter_b_notes': [ model_to_dict(n) for n in Note.objects.filter(assessment=Assessment.objects.get(fighter=matchup.fighter_b)).order_by('-createdAt')],
