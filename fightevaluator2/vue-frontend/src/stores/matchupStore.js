import { ref, inject, registerRuntimeCompiler } from 'vue'
...
export const useMatchupStore = defineStore('matchup', () => {
  ...
  function toggleActiveMatchUp(matchupId, tableName) {
    ...
    // console.log (matchupId,tableName,currActive);
    //prev active is clicked again to deactive
    if (currActive.id === parseInt(matchupId) && currActive.table === tableName){
        //deactive and return
        currActive.id = -1;
        currActive.table = null;
    ...
    if (currActive.id != -1){
      //deactivate
      let oldActive = getTableByName(currActive.table);
      oldActive[currActive.id].active = false;
